...
protected:
	 ...
	 */
	vector<Node_ptr> expand(Node_ptr node) const
	{
		vector<Node_ptr> childNodes;

		// Edges is a vector of pairs <state, action>
		vector<pair<shared_ptr<Action>, State_ptr>> actionStatePairs = node->getState()->getActions();
	 ...
	 * search iterations.
	 */
	Path_ptr startIteration(const State_type& initial)
	{
...
public:
	...
	Path_ptr search(const State_type& initial)
	{
		return startIteration(initial);
	}
	 ...
	 */
	template <class T = Success_functor, class = typename enable_if<is_same<T, IsGoalCheck<State_type>>::value>::type>
	Path_ptr search(const State_type& initial, const State_type& goal)
	{
